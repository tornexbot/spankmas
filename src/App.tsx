...
// Known PumpSwap pair address for SPANKMAS
const KNOWN_PAIR_ADDRESS = "7WZ1LJMg9HkCqTQNW8h5K8QQ7KcBx7JyPmU1J8XqKz3E";
...
/* ===================== BRIGHT RED BACKGROUND WITH GRID ===================== */

function BrightRedBackground() {
  return (
    <div className="fixed inset-0 overflow-hidden bg-gradient-to-b from-red-500 via-red-600 to-red-700 z-0">
      {/* Bright red gradient */}
      ...
      {/* Diagonal pattern - LIGHTER */}
      <div 
        className="absolute inset-0 opacity-10"
        style={{
          backgroundImage: 'linear-gradient(45deg, transparent 49%, rgba(255, 255, 255, 0.6) 50%, transparent 51%)',
      ...
      <div className="absolute inset-0">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80vw] h-[80vh] bg-gradient-to-r from-red-300/30 via-yellow-300/20 to-red-300/30 rounded-full blur-3xl" />
        <div className="absolute top-0 left-0 w-96 h-96 bg-gradient-to-br from-red-300/30 to-transparent blur-3xl" />
        <div className="absolute bottom-0 right-0 w-96 h-96 bg-gradient-to-tl from-red-300/30 to-transparent blur-3xl" />
      </div>
      
      {/* Vignette - LIGHTER */}
      <div className="absolute inset-0 bg-gradient-to-t from-red-600/10 via-transparent to-red-600/10" />
    </div>
  );
}
...
/* ===================== IMPROVED DEXSCREENER HOOK ===================== */

function useDexScreenerStats(pairAddress: string) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Use the pair endpoint directly for more accurate and frequent updates
      ...
      if (!result.pairs || result.pairs.length === 0) {
        // Fallback to token pairs if pair endpoint doesn't work
        ...
        // Find PumpSwap pair or highest liquidity
        const mainPair = tokenData.find((p: DexScreenerPair) => p.dexId === "pumpswap") ||
                         tokenData.sort((a: DexScreenerPair, b: DexScreenerPair) => 
                           (b.liquidity?.usd || 0) - (a.liquidity?.usd || 0)
                         )[0];
        
        setData(mainPair);
      } else {
        setData(result.pairs[0]);
      }
    } catch (err) {
...
/* ===================== FIXED DEXSCREENER STATS ===================== */

function DexScreenerStats() {
  const { data, loading, error, refetch } = useDexScreenerStats(KNOWN_PAIR_ADDRESS);

  if (loading) {
    return (
      <div className="rounded-xl border border-red-300/20 bg-white/95 p-6">
        <div className="flex h-48 flex-col items-center justify-center rounded-lg border border-zinc-200 p-8">
          <div className="h-10 w-10 animate-spin rounded-full border-4 border-red-600 border-t-transparent"></div>
          <p className="mt-4 text-sm text-zinc-600">Loading live market data...</p>
          <p className="mt-1 text-xs text-zinc-500">Connecting to DexScreener</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-xl border border-red-300/20 bg-white/95 p-6">
        <div className="flex h-48 flex-col items-center justify-center rounded-lg border border-zinc-200 p-8">
          ...
          <p className="mt-3 text-sm text-zinc-600">Market data temporarily unavailable</p>
          <p className="mt-1 text-xs text-zinc-500 max-w-xs text-center">{error || "No trading data found"}</p>
  ...
  const volume = data.volume?.h24 ? `$${(data.volume.h24).toLocaleString(undefined, { maximumFractionDigits: 0 })}` : "$0";
  const liquidity = data.liquidity?.usd ? `$${(data.liquidity.usd).toLocaleString(undefined, { maximumFractionDigits: 0 })}` : "$0";
  const change24h = data.priceChange?.h24 ? data.priceChange.h24.toFixed(2) : "0.00";
  const isPositive = data.priceChange?.h24 ? data.priceChange.h24 >= 0 : true;
  
  // FIXED: Accurate market cap logic - NO ESTIMATES
  let marketCap = "—";
  let marketCapLabel = "Market Cap";
  let marketCapSubtitle = "Fully diluted";

  if (data?.marketCap) {
    marketCap = `$${data.marketCap.toLocaleString(undefined, { maximumFractionDigits: 0 })}`;
  } else if (data?.fdv) {
    marketCap = `$${data.fdv.toLocaleString(undefined, { maximumFractionDigits: 0 })}`;
    marketCapLabel = "FDV (Fully Diluted)";
  } else {
    marketCap = "—";
    marketCapSubtitle = "Unavailable from DexScreener";
  }
  ...
  return (
    <div className="rounded-xl border border-red-300/20 bg-white/95 p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-zinc-600">Real-time data from DexScreener • Updates every 20s</p>
      </div>
      
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="rounded-lg border border-zinc-200 bg-white p-4">
          <div className="text-xs font-bold text-zinc-500">Current Price</div>
          <div className="mt-1 text-2xl font-black text-zinc-900">${price}</div>
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-4">
          ...
          <div className="mt-1 text-2xl font-black text-zinc-900">{volume}</div>
          <div className="mt-1 text-xs text-zinc-500">Trading activity</div>
        </div>
        
        <div className="rounded-lg border border-zinc-200 bg-white p-4">
          <div className="text-xs font-bold text-zinc-500">Liquidity</div>
          <div className="mt-1 text-2xl font-black text-zinc-900">{liquidity}</div>
          <div className="mt-1 text-xs text-zinc-500">Pool depth</div>
        </div>
        
        <div className="rounded-lg border border-zinc-200 bg-white p-4">
          <div className="text-xs font-bold text-zinc-500">{marketCapLabel}</div>
          <div className="mt-1 text-2xl font-black text-zinc-900">{marketCap}</div>
          <div className="mt-1 text-xs text-zinc-500">{marketCapSubtitle}</div>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <div className="text-xs font-bold text-zinc-500">Trading Pair</div>
            <div className="mt-1 text-sm font-bold text-zinc-900">
              {data.baseToken?.symbol || "SPANKMAS"} / {data.quoteToken?.symbol || "SOL"}
            </div>
          </div>
          <div>
            <div className="text-xs font-bold text-zinc-500">DEX</div>
            <div className="mt-1 text-sm font-bold text-zinc-900">
              {data.dexId ? data.dexId.toUpperCase() : "PumpSwap"}
            </div>
          </div>
          <div>
            <div className="text-xs font-bold text-zinc-500">Pair Address</div>
            <div className="mt-1 text-sm font-mono text-zinc-900 truncate">
              {data.pairAddress?.substring(0, 20)}...
            </div>
          </div>
          <div>
            <div className="text-xs font-bold text-zinc-500">Last Updated</div>
            <div className="mt-1 text-sm font-bold text-zinc-900">
...
function Nav({
  ...
  return (
    // FIXED: Changed z-40 to z-[9999] to ensure navbar is always on top
    <div className="fixed left-0 right-0 top-0 z-[9999] border-b border-white/10 bg-gradient-to-b from-red-600/95 to-red-700/90 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        <a href="#top" className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            <div className="text-[11px] font-semibold text-red-200">North Pole Meme Season</div>
          </div>
        </a>
  ...
  return (
    <AnimatePresence>
      {open && (
        ...
        >
          <motion.div
            className="absolute right-0 top-0 h-full w-[86%] max-w-sm bg-gradient-to-b from-red-600 to-red-700 shadow-xl"
            initial={{ x: 60 }}
            animate={{ x: 0 }}
            exit={{ x: 60 }}
          ...
          >
            ...
            <div className="px-4 py-4">
              ...
              <div className="mt-6 rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="text-xs font-bold text-red-200">Contract Address</div>
...
/* ===================== SECTION COMPONENT WITHOUT BACKGROUND ===================== */
...
function BrightSection({
  ...
  return (
    <section id={id} className="relative scroll-mt-24 px-4 py-12 sm:py-16 overflow-hidden">
      {/* Content container with higher z-index */}
      <div className="relative mx-auto w-full max-w-6xl z-30">
        <div className="max-w-3xl">
          {kicker && (
            <div className="inline-flex items-center gap-2 rounded-full border border-red-300/40 bg-red-500/20 px-3 py-1 text-xs font-bold text-red-100">
              <Sparkles className="h-4 w-4" />
              {kicker}
            </div>
          )}
          ...
          {subtitle && <p className="mt-3 text-sm sm:text-base text-red-50">{subtitle}</p>}
        </div>
...
function RealSpankmasLore() {
  return (
    ...
    >
      <div className="relative rounded-2xl border border-red-300/30 bg-gradient-to-b from-red-500/50 to-red-600/50 p-6 sm:p-8 shadow-2xl backdrop-blur-sm">
        <div className="relative space-y-6">
          ...
          <div className="space-y-4 text-lg leading-relaxed text-red-50">
            ...
            <div className="my-8 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
              <div className="flex items-center gap-2 rounded-full bg-gradient-to-r from-red-500/60 to-green-500/60 px-4 py-2 backdrop-blur-sm">
                <TreePine className="h-5 w-5 text-green-300" />
                <span className="text-sm font-bold text-white">Ancient Roots</span>
              </div>
              <div className="flex items-center gap-2 rounded-full bg-gradient-to-r from-green-500/60 to-red-500/60 px-4 py-2 backdrop-blur-sm">
                <Snowflake className="h-5 w-5 text-blue-300" />
                <span className="text-sm font-bold text-white">Modern Twist</span>
              </div>
              <div className="flex items-center gap-2 rounded-full bg-gradient-to-r from-red-500/60 to-yellow-500/60 px-4 py-2 backdrop-blur-sm">
            ...
            <div className="mt-10 rounded-2xl bg-gradient-to-r from-red-500/40 to-green-500/40 p-6 backdrop-blur-sm">
...
function HeroSection() {
  ...
  return (
    // FIXED: Removed overflow-hidden to prevent stacking context isolation
    <header className="relative min-h-screen px-4 pt-24">
      {/* Canvas Snow - only in hero */}
      <SnowCanvas />
      
      <div className="relative mx-auto max-w-6xl z-40">
        {/* Hero Content */}
        <div className="grid gap-8 lg:grid-cols-2 lg:gap-12 items-center">
          <div className="text-center lg:text-left">
            <div className="inline-block mb-4">
              <div className="inline-flex items-center gap-2 rounded-full border border-red-300/40 bg-red-500/20 px-4 py-2 text-sm font-bold text-red-100">
            ...
            <p className="text-lg sm:text-xl text-red-50 mb-8 max-w-xl mx-auto lg:mx-0">
            ...
            <div className="bg-gradient-to-r from-red-500/30 to-red-400/30 rounded-xl p-4 border border-red-300/30">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="text-sm font-bold text-red-100">Contract Address</div>
                <div className="font-mono text-xs text-white bg-red-500/40 p-2 rounded-lg truncate flex-1">
                  {CA}
                </div>
                <button
                  onClick={copyCA}
                  className="inline-flex items-center gap-2 rounded-lg border border-red-300/40 bg-red-500/30 px-4 py-2 text-sm font-bold text-white hover:bg-red-500/40"
                  type="button"
                >
                  {copied ? <Check className="h-4 w-4 text-green-400" /> : <Copy className="h-4 w-4" />}
                  {copied ? "Copied!" : "Copy"}
          ...
          <div className="relative">
            <div className="absolute -inset-4 -z-10 rounded-2xl bg-gradient-to-r from-red-400/20 to-yellow-400/20 blur-2xl"></div>
            <div className="rounded-2xl border border-red-300/30 bg-white/95 p-6 shadow-2xl backdrop-blur-sm">
...
export default function App() {
  ...
  return (
    <div id="top" className="min-h-screen bg-white text-zinc-900 relative overflow-x-hidden">
      {/* BRIGHT RED BACKGROUND - ONLY ONCE AT ROOT LEVEL */}
      <BrightRedBackground />
      ...
      >
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-xl border border-red-300/30 bg-gradient-to-br from-red-500/40 to-red-400/40 p-6 backdrop-blur-sm">
            <div className="text-xs font-bold text-red-100">Quick Actions</div>
            <div className="mt-4 space-y-3">
              <button
                type="button"
                className="w-full inline-flex items-center gap-2 rounded-lg border border-red-300/40 bg-red-500/30 px-4 py-3 text-sm font-bold text-white hover:bg-red-500/40"
                onClick={async () => {
                  try {
                    await navigator.clipboard.writeText(CA);
                  } catch (err) {
      ...
      >
        <div className="rounded-2xl border border-red-300/30 bg-gradient-to-br from-red-500/40 to-red-400/40 p-6 shadow-xl backdrop-blur-sm">
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="rounded-xl border border-red-300/30 bg-red-500/30 p-4 backdrop-blur-sm">
              <div className="text-xs font-bold text-red-100">Quick Links</div>
              <div className="mt-4 space-y-2">
                <a
                  href="#memes"
                  className="block w-full rounded-lg border border-red-300/40 bg-white/10 px-5 py-3 text-sm font-bold text-white hover:bg-white/20 text-center"
                >
                  View Media Gallery
                </a>
                <a
      ...
      <footer className="border-t border-white/10 bg-gradient-to-b from-red-600 to-red-700 px-4 py-8 relative z-30">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <div className="mt-1 text-xs text-red-200">© {new Date().getFullYear()} — The OG Christmas Meme Token</div>
            </div>
